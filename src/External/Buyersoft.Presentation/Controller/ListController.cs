...
public class ListController : ApiController
{
    ...
    public ListController(IMediator mediator, IBranchService branchService, IUserService userService, ITokenService tokenService, IDepartmentService departmentService) : base(mediator)
    {
    ...
    public async Task<IActionResult> GetAll([FromRoute] string entityName, [FromQuery] Dictionary<string, string> queryParameters = null)
    {
        ...
        if (entityName.ToLower() == "user")
        {
            ...
            var list = await _userService.GetSelectListItemAsync(companyId);

            return Ok(list);
        }

        if(entityName.ToLower() == "department")
        {
            var companyId = _tokenService.GetCompanyIdByToken();
            var list = await _departmentService.GetSelectListItemAsync(companyId);
