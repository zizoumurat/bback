...
public class OrderPreparationService : IOrderPreparationService
{
    ...
    public async Task CreateOrder(OrderCreateDto Model)
    {
        ...
        var orderFileContent = GeneratePurchaseOrderPdf(supplier, company, orderCode, Model.OrderItems.Sum(x => x.Quantity * x.UnitPrice), Model.OrderItems,Model.ShippingAddress,Model.Incoterms);
        ...
        var order = new Order()
        {
            ...
            DesiredShippingDate = Model.DesiredShippingDate,
            ...
            .Select(x => new OrderPreparationListDto()
            {
                ...
                OfferDetailList = x.Offer.OfferDetails.Select(od => new OfferDetailListDto(
                    ...
                    od.Quantity - x.Orders
                        ...
                        .Where(oi => oi.OfferDetailId == od.Id) 
                        .Sum(oi => oi.Quantity) 
                )).ToList(),
    ...
    private byte[] GeneratePurchaseOrderPdf(CompanyDetailDto supplier, CompanyDetailDto company, string PO, decimal totalPrice, List<OrderItemCreateDto> OrderItems, string shippingAddress, string incoterms)
    {
        ...
        var document = QuestPDF.Fluent.Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Row(row =>
                {
                    ...
                    row.ConstantItem(80).AlignRight().Height(50).Image(company.LogoContent).FitArea(); 
                });
                ...
                page.Content().Column(col =>
                {
                    col.Item().Row(row =>
                    {
                        row.RelativeItem()
                            ...
                            .Column(column =>
                            {
                                column.Item().PaddingTop(10).Text(company.Name).FontSize(18).Bold();
                            });
                    });



                    col.Item().PaddingTop(20).PaddingBottom(20).LineHorizontal(0.5f);
                    ...
                    col.Item().Row(row =>
                    {
                        row.RelativeItem(1).Border(1).Padding(10).Column(column =>
                        {
                            ...
                            column.Item().Text($"Adres : {supplier.Address}");
                            column.Item().Text($"Vergi Dairesi: {supplier.TaxAdministration}");
                            column.Item().Text($"Vergi Numarası: {supplier.TaxNumber}");
                        });


                        row.RelativeItem(1).Border(1).Padding(10).Column(column =>
                        {
                            column.Item().Text("PO Ref. No / Tarih").SemiBold().Underline();
                            column.Item().Text($"{PO} / {DateTime.Now.Date:dd/MM/yyyy}");

                            column.Item().Padding(5); 

                            column.Item().Text("İrtibat Kişisi").SemiBold().Underline();
                            column.Item().Text($"{company.ContactFirstName} {company.ContactLastName}");

                            column.Item().Padding(5);

                            column.Item().Text("İrtibat Telefon").SemiBold().Underline();
                            column.Item().Text(company.ContactPhoneNumber);
                        });
                    ...
                    col.Item().Row(row =>
                    {
                        row.RelativeItem(1).Border(1).Padding(10).Column(column =>
                        {
                            column.Item().Text("Teslimat Adresi").SemiBold().Underline();
                            column.Item().Text(shippingAddress).Bold();

                            column.Item().Padding(5);

                            column.Item().Text("Teslimat Şekli").SemiBold().Underline();
                            column.Item().Text(incoterms);
                        });
                    ...
                    col.Item().Table(table =>
                    {
                        ...
                        table.Header(header =>
                        {
                            ...
                            header.Cell().Element(CellStyle).Text("Net Birim Fiyat").AlignEnd().Bold();
                        });
                        ...
                        foreach (var item in OrderItems)
                        {
                            ...
                            table.Cell().Element(CellStyle).Text($"{item.UnitPrice.ToString("N2")} TRY / {item.Quantity}");
                            table.Cell().Element(CellStyle).Text($"{(item.UnitPrice * item.Quantity).ToString("N2")} TRY").AlignEnd();
                            orderNumber++;
                        }
                    ...
                    col.Item().Row(row =>
                    {
                        row.RelativeItem().Text("Toplam Tutar (KDV HARİÇ)").Bold();
                        row.RelativeItem().AlignRight().Text($"{totalPrice.ToString("N2")} (TRY)").Bold();
                    });
